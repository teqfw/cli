...
export async function createCliFixture(options = {}) {
 ...
 if (configurator) await source(root, 'src/Bootstrap/Container.mjs', `/** @implements {TeqFw_Cli_Api_Container_Configurator} */\nexport default class Container { configure(params) { globalThis.__fixtureConfigurator = params.applicationRoot; return {namespaceRoots: [], preprocessors: [], postprocessors: [], logging: false, configuration: {sources: [{id: 'fixture-config', load: async () => [{key: 'TEQFW_FIXTURE__VALUE', value: 'ready'}]}]}}; } }\n`);
 if (plugin) await source(root, 'src/Cli/Plugin.mjs', `/** @implements {TeqFw_Cli_Api_Plugin} */\nexport default class Plugin { constructor({reader}) { globalThis.__fixturePluginConfig = reader.get('TEQFW_FIXTURE'); return {onStartup() { (globalThis.__fixtureCalls ??= []).push('plugin:start'); }, onShutdown() { globalThis.__fixtureCalls.push('plugin:stop'); }}; } }\nexport const __deps__ = Object.freeze({default: Object.freeze({reader: 'TeqFw_Cfg_Reader$'})});\n`);
 await source(root, 'src/Cli/Command/Finite.mjs', `export default class Finite { constructor({io, reader}) { globalThis.__fixtureCommandConfig = reader.get('TEQFW_FIXTURE'); (globalThis.__fixtureCalls ??= []).push('command:finite:create'); return {id: 'fixture:finite', summary: 'Finish', lifetime: 'finite', arguments: [], options: [], execute: async ({launch}) => { globalThis.__fixtureCalls.push('command:finite:run'); globalThis.__fixtureLaunch = launch; io.write(JSON.stringify({cwd: launch.cwd, root: launch.applicationRoot}) + '\\n'); }}; } }\nexport const __deps__ = Object.freeze({default: Object.freeze({io: 'TeqFw_Cli_Adapter_Io$', reader: 'TeqFw_Cfg_Reader$'})});\n`);
