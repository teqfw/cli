...
export async function createCliFixture(options = {}) {
 ...
 await source(root, 'src/Cli/Command/Finite.mjs', `export default class Finite { constructor({io}) { (globalThis.__fixtureCalls ??= []).push('command:finite:create'); return {id: 'fixture:finite', summary: 'Finish', lifetime: 'finite', arguments: [], options: [], execute: async ({launch}) => { globalThis.__fixtureCalls.push('command:finite:run'); globalThis.__fixtureLaunch = launch; io.write(JSON.stringify({cwd: launch.cwd, root: launch.applicationRoot}) + '\\n'); }}; } }\nexport const __deps__ = Object.freeze({default: Object.freeze({io: 'TeqFw_Cli_Adapter_Io$'})});\n`);
