...
export async function createCliFixture(options = {}) {
 ...
 if (plugin) cli.plugin = 'Fixture_App_Plugin_Lifecycle$';
 ...
 if (configurator) await source(root, 'src/Bootstrap/Container.mjs', `/** @implements {TeqFw_Cli_Api_Container_Configurator} */\nexport default class Container { configure(params) { globalThis.__fixtureConfigurator = params.applicationRoot; globalThis.__fixtureConfiguratorArgv = params.argv; return {container: {namespaces: [], preprocessors: [], postprocessors: [], logging: false}, configuration: {sources: [{id: 'fixture-config', load: async () => [{key: 'TEQFW_FIXTURE__VALUE', value: 'ready'}]}]}}; } }\n`);
 if (plugin) await source(root, 'src/Plugin/Lifecycle.mjs', `/** @implements {TeqFw_Cli_Api_Plugin} */\nexport default class Lifecycle { constructor({reader, config}) { globalThis.__fixturePluginConfig = reader.get('TEQFW_FIXTURE'); globalThis.__fixtureRuntimeConfig = {root: config.applicationRoot, cwd: config.cwd, argv: [...config.argv], dotenvPath: config.dotenvPath, dotenvExplicit: config.dotenvExplicit}; return {onStartup() { (globalThis.__fixtureCalls ??= []).push('plugin:start'); }, onShutdown() { globalThis.__fixtureCalls.push('plugin:stop'); }}; } }\nexport const __deps__ = Object.freeze({default: Object.freeze({reader: 'TeqFw_Cfg_Reader$', config: 'TeqFw_Cli_Config$'})});\n`);
 ...
 await source(root, 'src/Cli/Command/Wait.mjs', `export default class Wait { constructor({io}) { (globalThis.__fixtureCalls ??= []).push('command:wait:create'); return {id: 'fixture:wait', summary: 'Wait', lifetime: 'long-running', arguments: [], options: [], start: async ({signal}) => { let resolve; const done = new Promise((r) => { resolve = r; }); signal.addEventListener('abort', resolve, {once: true}); globalThis.__fixtureResourceActive = true; io.write('started\\n'); return {done, stop: async () => { globalThis.__fixtureResourceActive = false; resolve(); }}; }}; } }\nexport const __deps__ = Object.freeze({default: Object.freeze({io: 'TeqFw_Cli_Adapter_Io$'})});\n`);
...
export function clearCliFixtureGlobals() {
 const keys = ['__fixtureConfigurator', '__fixtureConfiguratorArgv', '__fixturePluginConfig', '__fixtureCommandConfig', '__fixtureRuntimeConfig', '__fixtureCommandRuntimeConfig', '__fixtureLaunch', '__fixtureCalls', '__fixturePreprocessContexts', '__fixtureResourceActive'];
