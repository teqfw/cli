...
2. Keep pre-Container composition in the executable. The host owns the one module declared in `teqfw.fw.cli.container.configurator`; packages may publish compatible Configurator contributions for the host to statically import and merge, but must not declare that metadata. Its `container` result is JSON-safe policy data: namespace mappings and Dependency Identifiers for preprocessor, postprocessor, or hardener producers. The CLI supplies standard Sources, and plugins never create or configure a Container. A TeqFW package that declares canonical `teqfw.fw.di.namespaces`, `teqfw.fw.cli.commands`, and a dependency on `@teqfw/cli` may be launched as an explicit host with `teq --host <package> <command> [...]`; see [Usage](references/usage.md) for conventions, the option boundary, and host resolution.
