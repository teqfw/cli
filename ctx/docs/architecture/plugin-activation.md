...
TeqFw_Cli_Api_Plugin has exactly two methods: onStartup() and onShutdown(). Both may complete synchronously or return a promise. A class implementation uses `@implements {TeqFw_Cli_Api_Plugin}`; a function-form DI factory returning this structural value remains compatible. Configuration is already loaded before the component is resolved.
