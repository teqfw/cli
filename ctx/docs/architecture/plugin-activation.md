...
`TeqFw_Cli_Api_Plugin` is the public contract for an optional component that connects a TeqFW plugin package to a CLI application. It has exactly two methods: `onStartup()` and `onShutdown()`. Both may complete synchronously or return a promise. The component is distinct from the npm package: a package may declare one such component or none. Its exact metadata shape and JSDoc contract are owned by [command-contract.md](command-contract.md).

Bootstrap coordinates this phase but does not own signal plumbing or the shutdown stack. It opens one private Host session before resolving the first plugin component. The session subscribes to signals, retains each component whose `onStartup` has completed, invokes `onShutdown` for that retained stack, and exposes parser selection and command execution. Bootstrap uses the session to start each component in dependency-first package order, then to select the command. Thus a startup failure before command selection still closes every successfully started component.
...
Only Bootstrap directly triggers the two runtime resolution classes: CLI plugin components during startup and one selected command. The Host session, plugin components, commands, and all other products receive concrete values and ordinary declared dependencies only.
