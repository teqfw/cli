...
`@teqfw/di` provides namespace configuration and constructor graph resolution. `@teqfw/log` provides `TeqFw_Log_Provider$`; Host and Bootstrap bind stable sources and emit structured records with phase, participant, hook, signal, status, and error where appropriate.
